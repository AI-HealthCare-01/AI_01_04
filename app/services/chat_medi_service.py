...
class ChatMediService(BaseService):
    ...
    async def process_medical_chat(self, request: ChatRequest):
        # 1. 질병코드/질병명 → anchor 코드 + 질병명 + 가이드라인 조회
        raw_disease = request.disease_code or ""
        anchor_code, disease_name, guideline_texts = await self.disease_repo.resolve_disease_info(raw_disease)
        disease_name = disease_name or raw_disease
